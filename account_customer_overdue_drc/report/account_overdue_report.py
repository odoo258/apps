...
class ReportOverdue(models.AbstractModel):
    ...
    def _get_account_move_lines(self, partner_ids, date):
        ...
        self.env.cr.execute("SELECT m.name AS move_id, l.date, l.name, l.ref, l.date_maturity, l.partner_id, l.blocked, l.amount_currency, l.currency_id, "
                            ...
                            "WHERE l.partner_id IN %s AND at.type IN ('receivable', 'payable') AND NOT l.reconciled GROUP BY l.date, l.name, l.ref, l.date_maturity, l.partner_id, at.type, l.blocked, l.amount_currency, l.currency_id, l.move_id, m.name", (((date, ) + (tuple(partner_ids),))))
    ...
    def render_html(self, docids, data=None):
        ...
        lines = self._get_account_move_lines(docids, date)
        lines_to_display = {}
        ...
        for partner_id in docids:
            ...
            for line_tmp in lines[partner_id]:
                line = line_tmp.copy()
                currency = line['currency_id'] and self.env[
                    'res.currency'].browse(line['currency_id']) or company_currency
